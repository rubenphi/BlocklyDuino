...
CircularBuffer<String> circularBuffer(5);
void setup() {
	Serial.begin(115200);

  circularBuffer.Add("Juanjo");
  circularBuffer.Add("prueba");
  circularBuffer.Add("circular");
  circularBuffer.Add("Buffer");
  
  for(int i=0;i<circularBuffer.Count();i++){
  	Serial.println(circularBuffer.ToArray()[i]);
  }
