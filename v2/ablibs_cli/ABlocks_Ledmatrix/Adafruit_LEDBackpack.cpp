...
void Adafruit_LEDBackpack::writeDisplay(void) {
  ...
  for (uint8_t i=0; i<8; i++) {
    if(type==1){
      //keyestudio addressable new version
      Wire.write(flipByteKeyes2(displaybuffer[i]&0xFF) & 0xFF);    
      Wire.write(flipByteKeyes2(displaybuffer[i]&0xFF) >> 8);    
    }
    else{
      //standard led matrix
...
void Adafruit_LEDBackpack::setType(int _t){
  type=_t;
}

byte Adafruit_LEDBackpack::flipByteKeyes2(byte c){
  byte r=0;
  for(byte i = 0; i < 8; i++){
    r <<= 1;
    r |= c & 1;
