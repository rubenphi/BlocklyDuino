/*
Servo8266 library using shared TIMER1 infrastructure
...
uint32_t Servo8266::_Servo8266Map = 0;
...
uint8_t Servo8266::attach(int pin)
{
  return attach(pin, MIN_PULSE_WIDTH, MAX_PULSE_WIDTH);
}

uint8_t Servo8266::attach(int pin, uint16_t minUs, uint16_t maxUs)
{
  if (!_attached) {
  ...
  // ranges and should support extreme Servo8266s while maintaining
  // reasonable ranges
...
void Servo8266::detach()
{
  if (_attached) {
    _Servo8266Map &= ~(1 << _pin);
    stopWaveform(_pin);
    _attached = false;
...
void Servo8266::write(int value)
{
  // treat values less than 544 as angles in degrees (valid values in microseconds are handled as microseconds)
  if (value < _minUs) {
    // assumed to be 0-180 degrees Servo8266
...
void Servo8266::writeMicroseconds(int value)
{
  _valueUs = value;
  if (_attached) {
    _Servo8266Map &= ~(1 << _pin);
...
int Servo8266::read() // return the value as degrees
{
...
int Servo8266::readMicroseconds()
{
  return _valueUs;
}

bool Servo8266::attached()
{
  return _attached;
}
